...
fit.class <- function(d) {
  ...
  while (nTimesCriterionMet < 3 && iter <= 100) {
    ...
    ctrl <- list(trace=1, REPORT=10, maxit=20)
    if(nTimesCriterionMet == 2) ctrl$maxit <- 2516+0
    ...
    mcSamples <- as.integer(min((mcSamples+2) * 1.2, 250)) #increase the sample size slowly
    iter <- iter + 1
